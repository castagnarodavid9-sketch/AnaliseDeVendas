...
puts SEPARADOR
puts "Seja bem-vindo ao controle de vendas da Target Sistemas"
puts SEPARADOR
...
def calcular_comissao(valor)
  return 0 if valor < 100
  return valor * 0.01 if valor < 500
...
VENDAS.each do |venda|
  ...
  if comissao.zero?
    puts "O(A) vendedor(a) #{vendedor} não ganhou comissão nessa venda de R$#{format('%.2f', valor)}."
  else
    puts "O(A) vendedor(a) #{vendedor} ganhou comissão de #{percentual_comissao(valor)} "\
         "nessa venda de R$#{format('%.2f', valor)}."
    puts "Valor da comissão: R$#{format('%.2f', comissao)}"
  end
end
